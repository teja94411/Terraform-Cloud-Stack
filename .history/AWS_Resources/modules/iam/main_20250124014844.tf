...
resource "aws_iam_user" "new_iam_user" {
  for_each = toset(var.aws_iam_user)  # Convert list to set to avoid duplicates
...
resource "aws_iam_group_membership" "add_user_to_group" {
  ...
  group = aws_iam_group.group_name.name
  users = tolist([for user in aws_iam_user.new_iam_user : user.name])  # Extract list of usernames
}
